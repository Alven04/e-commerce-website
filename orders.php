...
<head>
  ...
  <title>American Football Equipment and Supplies : Orders</title>
  <!-- Bootstrap -->
  ...
  <div class="container-fluid">
    ...
    <form action="orders.php" method="post" class="form-horizontal">
      <div class="form-group">
          ...
          <div class="col-sm-9">
            <input name="oid" class="form-control" id="orderid" placeholder="Order ID" type="text" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_num']; ?>" required>
          </div>
      </div>
      <div class="form-group">
          <label for="odate" class="col-sm-3 control-label">Order Date</label>
          <div class="col-sm-9">
            <input name="orderdate" class="form-control" id="odate" placeholder="Order Date" type="text" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>" required>
          </div>
      </div>
    ...
    <table class="table table-striped table-bordered">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a186683, tbl_staffs_a186683_pt2, tbl_customers_a186683_pt2 WHERE ";
        $sql = $sql."tbl_orders_a186683.fld_staff_num = tbl_staffs_a186683_pt2.fld_staff_num and ";
        $sql = $sql."tbl_orders_a186683.fld_customer_num = tbl_customers_a186683_pt2.fld_customer_num";
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a186683");
            $stmt->execute();
            $result = $stmt->fetchAll();
